...
def rebuild_indices() -> MasterIndex:
    ...
    docs = [d for d in document_repository.list_documents() if d.status == "publicado"]
    ...
    for assunto, items in sorted(por_assunto.items()):
        ...
        index_repository.write_index(assunto, subj.model_dump())
    ...
    index_repository.write_index("_master", master.model_dump())
    return master
