...
**Front-end (React)** — o webapp é o front-end padrão (`webapp/`).
...
- Produção (servido pelo próprio FastAPI a partir de `webapp/dist`):
...
Veja `webapp/README.md` para todos os comandos (`lint`, `test`, `test:e2e`, `check`).

### Comandos de desenvolvimento (`backend/Makefile`)
...
| `Wikitor_CONTENT_ROOT` | `content` | Pasta dos markdown/índices |

## Estrutura

```
backend/app/      FastAPI em onion architecture (domain/application/infrastructure/presentation)
