...
export function useAssistant(onSaved: () => Promise<void>): AssistantState {
  ...
  // Identifica a sessão de chat para o histórico persistido no backend (content/chats/).
  // Gerado uma vez por sessão da aba; o backend gera o seu próprio se vier nulo.
  const [sessionId, setSessionId] = useState<string | null>(() => crypto.randomUUID());
